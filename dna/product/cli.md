...
- [id: CLI-PROD-PREVIEW-003] The bundled player exposes beeper audio after a browser user gesture and keeps emulation clocked when canvas rAF is suspended in a hidden tab; rendering may pause independently, and a long suspended backlog is dropped instead of replayed as fast-forward. [provenance: contract]
...
- [id: CLI-PROD-TRACE-001] `zxs trace` runs with instruction tracing; `--frames <n>` (default `5`), `--top <n>` (default `10`), `--last <n>` (default `50`), `--out <file>`, `--profile`, `--no-save`. `--profile` reports actual contended T-states, average T-states/percentage per 69,888-T-state frame, grouped by nearest preceding SLD function label, plus separate HALT-idle and interrupt-acknowledge shares; the label attribution is explicitly heuristic. [provenance: contract]
...
- [id: CLI-PROD-TEST-001] `zxs test [path]` runs declarative asm tests from a directory or `*.test.json` spec (default `.`); specs may apply memory `setup`, wait for `memEquals` readiness, and assert `frameBudget` in addition to the temporal/checkpoint + memory delta/range + `screenDiff` set specified in `recipes-and-assertions.md`. `--list-assertions` prints all 17 assertion types. [provenance: contract]
- [id: CLI-PROD-DOCTOR-001] `zxs doctor` checks the toolchain: Node ≥ 20, embedded `@zx-vibes/asm` importable, the 48K ROM present (16384 bytes), and `zxs` PATH resolution. The PATH check groups npm's extensionless/`.cmd`/`.ps1` wrappers by canonical package root, but fails for multiple distinct installations or a resolved package missing `dist/cli.js`. `sjasmplus` is checked only when configured as the escape-hatch backend (ADR-0027 D3), not by default. [provenance: decision:ADR-0027]
...
- [id: CLI-PROD-OUT-RUN-001] `run --json` reports `{ ok, stage: "run", status, boot, exit: { reason, pc }, framesRun, tstatesRun, haltSynced, frameBudget, audio, registers, screen, input }` where `status` is one of `ok`, `hang`, `breakpoint`, `watchpoint`; `frameBudget` carries `{ frameTStates, measuredFrames, interruptFrames, overrunFrames, idleTStates: { min, average }, worstFrame }`. [provenance: contract]
