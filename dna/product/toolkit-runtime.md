...
- [id: RT-PROD-PREVIEW-006] The browser player renders the audible speaker drive — the weighted EAR+MIC port-`0xFE` mix, `beeper-output.md` BEEPER-PCM-MIX-001 — through Web Audio after a user gesture, so game beepers (`b4`) and the ROM `SAVE`'s MIC-only tone (`b3`) are both audible. Its PCM sample grid and filter state are continuous across frames, use the exact 3.5 MHz/69,888-T-state cadence, and do not accumulate rounded samples-per-frame drift. [provenance: contract]
...
- [id: RT-PROD-PREVIEW-008] The browser player renders the **320×240 bordered frame** of `raster-border.md` RASTER-GEOMETRY-001 — the 256×192 display inset by the visible 32-px/24-line border — as canvas pixels, not page styling. The border colour varies **per scanline**: each visible row takes the colour in effect where its raster line begins, from the frame's chronological border events stamped on the monotonic total-T-state clock (the row-granular reading of RASTER-TSTATE-PIXEL-001, with the colour at the top of a frame carried from the previous frame), so tape `SAVE`/`LOAD` paints its red/cyan bands (RASTER-SAVE-PP-001) instead of a whole-frame flicker. Symbol characters type per `keyboard-input.md` KBD-BROWSERMAP-002. [provenance: decision:ADR-0028]
