...
const pairs = [
  {
    name: 'docs/reference',
    src: join(root, 'docs', 'reference'),
    dest: join(pkg, 'docs', 'reference'),
  },
  {
    name: 'docs/agents/skills',
    src: join(root, 'docs', 'agents', 'skills'),
    dest: join(pkg, 'docs', 'agents', 'skills'),
  },
];
...
if (checkOnly) {
  ...
  if (drift.length) {
    throw new Error(`Synced toolkit docs are stale:\n${drift.map((line) => `- ${line}`).join('\n')}`);
  }
  process.exit(0);
}
...
function assertSourceExists({ name, src }) {
  if (!existsSync(src)) throw new Error(`Missing source docs directory: ${name}`);
  if (!statSync(src).isDirectory()) throw new Error(`Source docs path is not a directory: ${name}`);
}
