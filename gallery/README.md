...
The gallery itself has no build step. Serve the directory over HTTP
(`file://` won't work because the pages fetch JSON/snapshots):

```bash
npx serve gallery          # or: python3 -m http.server -d gallery 8080
```

GitHub Pages: this repository's Pages workflow builds the VitePress manual into
`gallery/manual/`, then publishes the `gallery/` directory. Enable Pages with
GitHub Actions as the source in the repository settings before relying on the
public URL.
